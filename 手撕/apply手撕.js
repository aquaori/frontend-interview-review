...
核心逻辑：
1.参数解构：第一个参数是context（也就是this对象），第二个参数是argsArray，应该是个数组
2.边界处理：context如果是null或undefined，就把它指向全局对象globalThis，如果argsArray不是数组或者没传，就当成空数组处理，避免报错
3.原始值包装：用Object()将context包装成对象，这样即便是数组、字符串也能挂载方法，在调用最终函数时，使用扩展运算符...argsArray将数组展开成参数列表
